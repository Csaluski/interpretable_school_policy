  ...
  if (heatmap) {
    ...
    for (colx in heatmap.cols) {
      for (coly in heatmap.cols) {
        ...
        print(min(original_dt[[colx]]))
        print(max(original_dt[[colx]]))
...
ic.dt <- ic.dt[joined.dt[, .(State.District.ID, year, ETLP_avg)], on=c("State.District.ID", "year")]
...
coaching.agg.dt <- coaching.agg.dt[joined.dt[, .(State.District.ID, year, ETLP_avg)], on=c("State.District.ID", "year")]
